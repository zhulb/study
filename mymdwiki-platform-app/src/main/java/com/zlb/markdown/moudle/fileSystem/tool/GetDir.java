 ...
 */
public class GetDir {
	public static final String FILE_PATH2 = "_url";
	...
	public JSONObject getDirLoop(String parentPath){
		...
		result.put(FILE_PATH2, parentPath);
		result.put(FILE_PATH, "/");
		result.put("name", new File(parentPath).getName());
		int cur = 0;
		...
		while(size>cur){//给target链表的每一个文件夹对象添加children属性，并且target链表可以动态追加
			...
			File curFile=new File(target.get(cur).getString(FILE_PATH2));
			...
			for(j=0;j<childrenFile.length;j++){
				...
				if(!child.isFile()){//文件夹
					...
					childJSONObject.put(FILE_PATH2, child.getAbsolutePath());
					childJSONObject.put(FILE_PATH,curTarget.getString(FILE_PATH)+child.getName()+"/");
					childJSONObject.put("name",child.getName());
